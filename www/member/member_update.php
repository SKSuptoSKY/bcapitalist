...
referer_check();
	...
	if($URL==FALSE) $URL = "/member/member_form.php?mode=INFO";
...
if($mode=="BREAK") {
	...
	if($Gmember==TRUE)
	{
		...
		{
			alert_close("비밀번호가 맞지 않습니다.", "/member/logout.php");
		}
	...
	sess_kill();
	alert_close("그동안 이용해 주셔서 감사합니다.", "/main.php");
}
//////////////////////// 회원 탈퇴 //////////////////////////
//////////////////////////////////////////////////////////////
...
referer_check();
	...
	if($default["mail_check"]==TRUE) {
		...
		$mail_check_href = $default["site_url"]."/member/member/login.php?PasCode=$Checkpass&URL=";
	} else {
		// 회원인증 일자를 강제로 넣어줍니다.
	...
	if($URL==FALSE) $URL = "/member/login.php";
