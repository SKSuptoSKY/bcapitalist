...
if($mode=="INFO") {
	//세션이 없으면 멤버 폴더로 이동
	if($_SESSION["userlevel"]<10) alert("로그인후 이용하실 수 있습니다.", "/mobile/member/login.php?URL=/mobile/member/member_form.php?mode=INFO");
	...
	//// 올바른 접속 경로를 확인하고 되돌려줍니다 
	 if($mem==FALSE) {
		 sess_kill();
		alert("로그인 정보가 없습니다.", "/mobile/main.php");
	}
...
//스킨페이지를 불러옵니다.
include $_SERVER["DOCUMENT_ROOT"]."/mobile/skin/member/{$G_member[skin_dir]}/member_form.skin.php";
?>