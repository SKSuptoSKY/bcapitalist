<?
if($pg_no != 5) $page_loc = "site";
else $page_loc = "lecture";

include "../head.php";

$PG_table = $GnTable["pageitem"];

if($pg_no != 5) $title_page = "페이지";
else $title_page = "게좌안내";

if($pg_no != 5) $cate_array = array('회사소개','강의','회사소개','강의');
			...
			<td style="padding-left:10px;">
				<table width="100%" cellpadding="6" cellspacing="1" border="0" bgcolor="#cccccc">
					<tr>
						...
						<td colspan=2 bgcolor="#F7F7F7" align="center" style="<?=($pg_no == 1 or $pg_no == 2)?'font-weight:bold;color:#000082':'';?>">국문</td>
						<td colspan=2 bgcolor="#F7F7F7" align="center" style="<?=($pg_no == 3 or $pg_no == 4)?'font-weight:bold;color:#000082':'';?>">영문</td>
					</tr>
					<tr>
						<?for($i=0; $i < count($cate_array); $i++){?>
							<td bgcolor="#ffffff" align="center" style="<?=($pg_no == ($i+1))?'font-weight:bold;color:#000082':'';?>">
							<a href="<?=$_SERVER[PHP_SELF]?>?mode=E&pg_no=<?=($i+1)?>"><?=$cate_array[$i]?></a></td>
						<?}?>
					</tr>
				</table>
			</td>
...
<table width="750px" align="center" border="0" cellpadding="3" cellspacing="1" bgcolor="#E0E0E0">
<colgroup width=100>
<colgroup width="">
	<tr bgcolor="#FFFFFF">
		...
		<?}else{?>
		<input type="hidden"  name="pg_subject" value="<?=$it[pg_subject]?>">
		<?=$it[pg_subject]?>
		<?}?>
		</td>
	</tr>
