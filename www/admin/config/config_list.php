...
function chkDel(code) {
    if(confirm("게시판을 삭제하시면 복구하실 수 없습니다.\n\n삭제하시겠습니까?")) 
	document.location.href = "./config_update.php?mode=D&page=<?=$page?>&code=" +code;
}
function BoardClear(code) {
	yes_no = confirm('게시판의 모든 글이 삭제됩니다.\n\n게시판을 비우시겠습니까?');
	if(yes_no == true) { // 확인 선택해 했을때 
		location.href='./config_update.php?mode=C&page=<?=$page?>&code='+code;
	} 
} 
</script>
		...
		<td>
			<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#E0E0E0">
				<tr align="center" bgcolor="#F6F6F6"> 
					<td width=100><a href="./config_form.php?mode=W&ca_id=<?=$ca_id?>"><img src="/btn/btn_newup.gif" border=0></a></td>
					<td>테이블명</td>
					<td>게시판제목</td>
					<td>메뉴명</td>
					<td>스킨명</td>
			...
			<? for ($i=0; $i<$list_total; $i++) { ?>
				<tr align="center" bgcolor="#FFFFFF"> 
					<td style="font-weight:bold;">
						<a href="./config_form.php?mode=E&id=<?=$list[$i][code]?>&page=<?=$page?>"><font color="#0033FF">수정</font></a> / 
						<a href="javascript:chkDel('<?=$list[$i][dbname];?>')"><font color="#FF3300">삭제</font></a>
					</td>
					<td><?=$list[$i][dbname]?></td>
