...
if($mode=="E") {
	...
	if($view[b_no]==FALSE) alert("등록된 자료가 없습니다.");
		...
		$sql = " select b_tno, b_dep, b_subject, b_category, b_content, b_ex1, b_ex2, b_secret from $PG_table where b_no = '$b_no' ";
		$view = sql_fetch($sql);
		...
		$view["b_ex1"] = $view["b_ex1"];
		$view["b_ex2"] = $view["b_ex2"];
}
					...
					<td><table width="100%" cellpadding="0" cellspacing="0" border="0">
						<?for($i=1; $i <= 1; $i++){?>
							<tr>
								<td align="left">
									<?if($i == 1){?>
									이미지 업로드 권장사이지 : <span style="color:#FF0000"></span>
								<br>
								<? } ?>
...
<table width="100%">
	<tr>
		<td align=center height=50>
			...
			<?if($mode != "R"){?>
				<a href="/admin/item_review/form.php?mode=R&b_no=<?=$view[b_no]?>&<?=$qstr?>"><img  src="/btn/btn_reply.gif" style="border:0px;"></a>
			<? } ?>
		</td>
	</tr>
</table>
