...
/* -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
|	오픈컴, 외부호스팅 DB 서버 구분 커넥션 - Start
|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
|
|	Web Server IP >>> DB Server IP
|   ------------------------------------------------------------------
|	"121.78.88.90" >>> 121.78.88.97:3307  ( mysql4 버젼)
|	"121.78.88.91" >>> 121.78.88.97:3307  ( mysql4 버젼)
|	"121.78.88.92" >>> 121.78.88.97
|	"121.78.88.93" >>> 121.78.88.97
|	
|	2015 - MJ  */

// 오픈컴 Web서버 IP 목록
$opencom_webserver_ip_array = array(
	"121.78.88.90",
	"121.78.88.91",
	"121.78.88.92",
	"121.78.88.93",
...
// 현재 접속 서버가 오픈컴 서버인지 검사
if( in_array($_SERVER["SERVER_ADDR"], $opencom_webserver_ip_array) ) 
{
	// 오픈컴 DB 서버 IP	
	$host_ip = "121.78.88.97";

	// 90번, 91번 웹서버는 97디비서버 접속시 3307 포트 사용 (mysql 4버전)
	if( $_SERVER["SERVER_ADDR"]=="121.78.88.90" || $_SERVER["SERVER_ADDR"]=="121.78.88.91" ) {
		$port_num = "3307";
	} else {
		$port_num = "";
	}
...
/* -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
|	오픈컴, 외부호스팅 DB 서버 구분 커넥션 - End
|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/

//$cfg['Servers'][$i]['host']        = 'localhost';		// MySQL hostname or IP addressj
$cfg['Servers'][$i]['host']          = $host_ip;			// MySQL hostname or IP addressj
$cfg['Servers'][$i]['port']          = $port_num;		// MySQL port - leave blank for default port
