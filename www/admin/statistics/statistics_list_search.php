...
//여기까지 날짜설정=====================================================================================
	...
	if ($search_02 != "")
	{
		if(strlen($search_star_day) == 1) $search_star_day = "0".$search_star_day;
		if(strlen($search_end_day) == 1) $search_end_day = "0".$search_end_day;

		if(strlen($search_star_month) == 1) $search_star_month = "0".$search_star_month;
		if(strlen($search_end_month) == 1) $search_end_month = "0".$search_end_month;
		
		$search_star = $search_star_year."-".$search_star_month."-".$search_star_day;
		$search_end = $search_end_year."-".$search_end_month."-".$search_end_day;
		if($search_02 == "1"){ // 결제일 기준
		 ...
		 $where_month = $month;
		if($search_op01 == "무통장입금"){
			 $sql_search .= " $where od_receipt_bank  != ''";
		}else if($search_op01 == "카드결제"){
			 $sql_search .= " $where od_receipt_card != ''";			
		}else{
	...
	$qstr = "$qstr1&sort1=$sort1&sort2=$sort2&page=$page&search_op01=$search_op01&search_02=$search_02&search_star_year=$search_star_year&search_star_month=$search_star_month&search_star_day=$search_star_day&search_end_year=$search_end_year&search_end_month=$search_end_month&search_end_day=$search_end_day";
...
<tr>
				...
				<td bgcolor="#ffffff" style="padding-left:10px;">
					<input type="radio" name="search_op01" id="search_op01_01" value="" <?if(!$search_op01) echo "checked";?>>전체
					<input type="radio" name="search_op01" id="search_op01_02" value="무통장입금" <?if($search_op01 == "무통장입금") echo "checked";?>>무통장입금
					<input type="radio" name="search_op01" id="search_op01_03" value="카드결제" <?if($search_op01 == "카드결제") echo "checked";?>>카드결제
				</td>
			</tr>
			<tr bgcolor="#F6F6F6">
						...
						<tr>
							<td width="100">
								<select name="search_02">
									<option value="1" <?=($search_02 == "1")?"selected":"";?>>결제일기준</option>
									<option value="2" <?=($search_02 == "2")?"selected":"";?>>주문일기준</option>
								</select>
							</td>
							<td width="215">
								<select name="search_star_year">
								<?
									if(!$search_star_year) $search_star_year = date("Y");
									for($i=2004; $i <= date("Y"); $i++){
								?>
									<option value="<?=$i?>" <?=($i==$search_star_year)?"selected":""?>><?=$i?></option>
								<?
								}
								?>
								</select> 년
								...
								<?
									if(!$search_star_month) $search_star_month = date("m");
									for($i=1; $i <= 12; $i++){
								?>
									<option value="<?=$i?>" <?=($i==$search_star_month)?"selected":""?>><?=$i?></option>
								<?
								}
								?>
								</select> 월
								...
								<?
									if(!$search_star_day) $search_star_day = 1;
									for($i=1; $i <= 31; $i++){
								?>
									<option value="<?=$i?>" <?=($i==$search_star_day)?"selected":""?>><?=$i?></option>
								<?
								}
								?>
								</select> 일 ~ 
							...
							<td width="215">
								...
								<?
									if(!$search_end_year) $search_end_year = date("Y");
									for($i=2004; $i <= date("Y"); $i++){
								?>
									<option value="<?=$i?>" <?=($i==$search_end_year)?"selected":""?>><?=$i?></option>
								<?
								}
								?>
								</select> 년
								...
								<?
									if(!$search_end_month) $search_end_month = date("m");
									for($i=1; $i <= 12; $i++){
								?>
									<option value="<?=$i?>" <?=($i==$search_end_month)?"selected":""?>><?=$i?></option>
								<?
								}
								?>
								</select> 월
								...
								<?
									if(!$search_end_day) $search_end_day = 31;
									for($i=1; $i <= 31; $i++){
								?>
									<option value="<?=$i?>" <?=($i==$search_end_day)?"selected":""?>><?=$i?></option>
								<?
								}
								?>
								</select> 일 
		...
		<td>
			<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#E0E0E0" style="word-break:break-all;">
				...
				<tr align="center" bgcolor="#F6F6F6"> 
					<td>날짜</td>
					<td>검색 된 총매출[<b><span id="tot_amount_view">0</span>원</b> <b>(<span id="tot_count_view">0</span> 건)</b>] </td>
					<td width="180">매출액</td>
					<td width="180">PG</td>
					<td width="180">현금</td>
					<td width="180">포인트</td>
