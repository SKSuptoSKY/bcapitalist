...
class Smtp {
	...
	function Smtp($host="121.78.91.210") {
	...
	function smtp_connect($host) {
		...
		$this->dialogue(250, "HELO smtp210.gamgakdesign.com");
		return true;
	}
	...
	// 메시지를 보낸다
	function smtp_send($to, $from, $data) {
		$from = iconv("UTF-8","EUC-KR",$from);
		$to = iconv("UTF-8","EUC-KR",$to);
		$email_exp   = explode('|', $to);
		$from_exp = explode('|', $from);
		if(!$mail_from = $this->get_email($from_exp[1])) return false;
		if(!$rcpt_to = $this->get_email($email_exp[1])) return false;
		($email_exp[1])   ? $ToStr = "{$email_exp[0]} <{$email_exp[1]}>" : $ToStr = "$email_exp[1]";
		if(!$this->dialogue(250, "MAIL FROM:$mail_from"))
		$this->error[] = $email_exp[0].":MAIL FROM 실패($this->lastmsg)";
		if(!$this->dialogue(250, "RCPT TO:$rcpt_to"))
		$this->error[] = $email_exp[0].":RCPT TO 실패($this->lastmsg)";
		$this->dialogue(354, "DATA");
		$mime = "Message-ID: <".$this->get_message_id().">\r\n";
		$mime .= "From: \"$from_exp[0]\" <$from_exp[1]>\r\n";
		$mime .= "To: $ToStr\r\n";
		fputs($this->fp, $mime);
		fputs($this->fp, $data);
		$this->dialogue(250, ".");
