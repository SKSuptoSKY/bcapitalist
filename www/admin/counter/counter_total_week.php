<?
	...
	if (empty($fr_date)) $fr_date = date("Y-m-",$now)."01";
	...
	// 출력할 레코드를 얻음
	$sql = " select WEEKDAY(con_date) as week, count(con_id) as count from $PG_table $sql_search group by week order by week ";
	$result = sql_query($sql,FALSE);
	$weekday = array ('월', '화', '수', '목', '금', '토', '일');

	for ($i=0; $row=sql_fetch_array($result); $i++) {
		$arr[$row[week]] = $row["count"];
	}

	for ($i=0; $i<7; $i++) {
		$list[$i]["weekday"] = $weekday[$i];
		$list[$i]["count"] = (int)$arr[$i];
...
<table width="99%" border="0" cellspacing="0" cellpadding="0" align="center">
	...
	<tr>
		<td height="30" bgcolor="#F5F5F5" style="padding-left:5px;">
			<strong><font color="#004080"><img src="/admin/images/title_icon.gif" width="10" height="9"> 요일별 접속자 통계</font></strong>
		</td>
