			...
			<input type="hidden" name="list_num[]" value="<?=$list[$i][b_no]?>">
				<tr bgcolor="#FFFFFF"> 
					<td style="font-weight:bold;text-align:center">
						<a href="./form.php?mode=E&b_no=<?=$list[$i][b_no]?>&<?=$qstr?>"><font color=#0033FF>수정</font></a> / 
						<a href='javascript:del("./update.php?mode=D&b_no=<?=$list[$i][b_no]?>&<?=$qstr?>")'><font color=#FF3300>삭제</font></a>
					</td>
					<td  style="padding:10px;"><?=($list[$i][b_dep] == "A")?"[<a href='/shop/item.php?it_id=".$list[$i][b_ex2]."' target='_blank'>".$list[$i][b_ex3]."</a>] ":"";?><?=$list[$i][subject]?></td>
