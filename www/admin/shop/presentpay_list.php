...
$type = "0";   ///////////// 타입을 구분합니다. 0 => 구매가격별 증정품 , 1 => 개별상품별 증정품
...
$sql_order = "order by $sst $sod";
	...
	$page_title = "<b>구매 가격 증정품</b>&nbsp;&nbsp;&nbsp;&nbsp;<a href='./presentitem_list.php?type=1'>개별 상품 증정품</a>";
	$sql_common = "from $PG_table a, $JO_table b where a.pr_type = " . $type ." and b.it_id = a.pritem_num";

// 출력할 레코드를 얻음
$sql  = " select a.* , b.it_name as pritem_name 
           $sql_common
           $sql_order
...
<input type=hidden name=type value='<? echo $type ?>'>
	<tr align="center" bgcolor="#F6F6F6">
		...
		<td width='' height=25><?=subject_sort_link("pritem_name");?>증정품</a></td>
		...
		<td width=60 align=center><?=subject_sort_link("pr_state");?>상태</a></td>
	</tr>
<? for ($i=0; $i<$list_total; $i++) { ?>
	...
	<input type=hidden name='pr_id[<?=$i?>]' value='<?=$list[$i][pr_id];?>'>
		...
		<td align=left><a href='./item_form.php?mode=E&it_id=<?=$list[$i][pritem_num]?>'><?=$list[$i][pritem_name]?></a></td>
		<td align=center><input type=text name='odto_pay[<?=$i?>]' value='<?=number_format($list[$i][odto_pay])?>' title='<?=$list[$i][pr_id]?>' required itemname='분류명' size=10></td>
		<td align=center><input type=text name='pr_num[<?=$i?>]' value='<?=number_format($list[$i][pr_num])?>' title='<?=$list[$i][pr_num]?>' required itemname='증정갯수' size=5></td>
