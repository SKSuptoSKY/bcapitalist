...
<colgroup width="">
	...
	<tr bgcolor="#FFFFFF">
		...
		<td>
	<? if ($mode == "W") { // 추가 ?>
		<input type=hidden name="it_id" value="<?=$it_id?>"><?=$it_id?>
...
<?
// -------------------------------------------------------------------------------------------- [ 옵션 제어 /admin/config/menu_list.php 연동 - Start ] ------------------------------------------------------------------------------------------
if ($sitemenu["mn_shop_option_use"]) 
{
	// 구분자로 저장된 상품 옵션 체크내역을 배열로 저장
	$option_type_arr = @explode("|",$sitemenu["mn_shop_option_type"]);
	?>
	<tr bgcolor="#FFFFFF">
		...
		<td>
			...
			<? if ($option_type_arr[0]) { ?><input type="radio" name="it_opt_use" value="1" <? if ($it[it_opt_use]=="1") { ?>checked<? } ?> onclick="option_view('1')">사용(단일구입옵션) <? } ?>
			<? if ($option_type_arr[1]) { ?><input type="radio" name="it_opt_use" value="2" <? if ($it[it_opt_use]=="2") { ?>checked<? } ?> onclick="option_view('2')">사용(다중구입옵션) <? } ?>
			<? if ($option_type_arr[2]) { ?><input type="checkbox" name="it_opt_use2" value="1" <?=($it[it_opt_use2] == "1")?"checked":"";?>>다중 원가포함 <? } ?>
		</td>
	</tr>
...
{
	// 관리자 설정에서 옵션 사용안함으로 했을시 무조건 옵션은 사용안함 형태
	?><input type="hidden" name="it_opt_use" value=""><?
}
// -------------------------------------------------------------------------------------------- [ 옵션 제어 /admin/config/menu_list.php 연동 - End ] ------------------------------------------------------------------------------------------ 
?>
	...
	<? if ($sitemenu["mn_shop_related_use"]) { ?>
	<tr bgcolor="#FFFFFF">
...
<table width="99%" align="center" border="0" cellpadding="3" cellspacing="1" bgcolor="#E0E0E0">
	...
	<? for ($i=1; $i<=$GnShop["max_img_count"]; $i++) { ?>
		<tr bgcolor="#FFFFFF">
			<td bgcolor="#F0F0F0" style="width:150px; padding-left:10px">이미지(대) <?=$i?></td>
			<td>
				...
				$limg = $GnShop["shop_item_dir"]."/".$file_name[$i]["small"];

				if ( $file_name[$i]["small"] ) {
					$limg= img_resize_tag($GnShop["shop_item_url"]."/".$file_name[$i]["small"],30,30);
