<?php
	...
	$LGD_MERTKEY = $GnShop["LG_pg_key"];  //LG텔레콤에서 발급한 상점키로 변경해 주시기 바랍니다.
        ...
        if ( "0000" == $LGD_RESPCODE ){ //결제가 성공이면
        	if( "R" == $LGD_CASFLAG ) {
				...
				$sql = " UPDATE {$GnTable[shoporder]}
							set od_deposit_name	= '$LGD_PAYER',
								od_receipt_bank		= '$LGD_CASTAMOUNT',
								od_bank_time		= '$LGD_PAYDATE',
								od_bill					= '$LGD_CASHRECEIPTKIND',
								od_billcode			= '$LGD_CASHRECEIPTNUM',
				...
				// 장바구니의 주문상태 변경
				$sql = "update {$GnTable[shopcart]} set ct_status = '준비' where ct_od_id = '$LGD_OID' ";
				sql_query($sql);
				##################   [ 무통장 입금 성공 DB 처리 - END ]  ##################
				...
				// 장바구니의 주문상태 변경
				$sql = "update {$GnTable[shopcart]} set ct_status = '주문' where ct_od_id = '$LGD_OID' ";
				sql_query($sql);
				##################   [ 무통장 입금 취소 DB 처리 - END ]  ##################
        	}
        } else { //결제가 실패이면
