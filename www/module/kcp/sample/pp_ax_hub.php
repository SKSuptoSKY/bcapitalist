...
<?
    ...
    {   
			...
			$sql = "select lec_pay from Gn_Lecture where lec_no = '$_POST[order_lec]'";
			$lec = sql_fetch($sql);
			$lec[lec_pay] = $lec[lec_pay] * 1.1;
			$c_PayPlus->mf_set_ordr_data( "ordr_mony",  $lec[lec_pay] );
	...
	if ( $req_tx == "pay" )
    {
		if( $res_cd == "0000" )
        {
			...
			if ( $use_pay_method == "100000000000" )
            {
				include $_SERVER["DOCUMENT_ROOT"]."/gamgak_card_update.php";
			...
			if ( $use_pay_method == "010000000000" )
            {
				include $_SERVER["DOCUMENT_ROOT"]."/gamgak_bank_update.php";
			}
			// 06-1-3. 가상계좌
			if ( $use_pay_method == "001000000000" )
            {
