...
<div class="bbs_gallery_Wrap">
	<ul class="bbs_gallery clfix">
	<?
	for ($i=0; $i<$list_total; $i++)
	{
		...
		<li>
			<a href="<?=$list[$i]["viewUrl"]?>">
				<div class="gall_pic"><img src="<?=$list[$i]["img_1"]?>" style="width:250px; height:142px"></div>
				<div class="gall_txt">
					<strong><?=cut_str($list[$i]["subject"], 30, "...")?></strong>
					<p><?=cut_str(strip_tags(stripslashes($list[$i]["b_content"])),110,"...")?></p>
					<span class="w_date">작성일 <span class="sep">|</span> <?=substr($list[$i]["b_regist"],0,10)?></span>
				</div>
				<img src="/images/btn_more.jpg" alt="" class="btn_more_story" />
			</a>
		</li>
		<?
	}
	...
	{
		?>
			<div style="display: table; width: 900px; height: 300px; #position: relative; overflow: hidden;">
...
<div class="paging_wrap">
	<ul class="paging mt30">
		<?=custom_paging_story2($default[page_list] ,$page, $total_page, "$_SERVER[PHP_SELF]?tbl=$Table&$PageNext&$NextUrl&page=") ?>
	</ul>
</div>
...
<style>
.bbs_gallery{box-sizing:border-box; }
.bbs_gallery:before, .bbs_gallery:after{box-sizing:inherit;}
.bbs_gallery li{width:280px; height:300px; float:left; margin-left:20px; margin-bottom:40px; border:1px solid #dddddd; position:relative; }
.bbs_gallery li:nth-child(3n+1){margin-left:0; }
.bbs_gallery li a{}
.gall_pic{width:280px; height:170px; display:table-cell; text-align:center; vertical-align:middle; }
.gall_pic img{max-height:142px; max-width:250px; }
.gall_txt{padding:5px 20px; }
.gall_txt strong{font-size:14px; color:#333333; font-weight:bold; display:block;  margin-bottom:5px; }
.gall_txt p{font-size:13px; color:#666666; }
span.w_date{display:block; font-size:12px; color:#666666; margin-top:5px; text -align:right; }
span.sep{margin:0 5px; }
img.btn_more_story{position:absolute; right:0; bottom:0; }
.bbs_gallery li:hover{background:#304878; border:1px solid #0d2f91; }
.bbs_gallery li:hover strong{color:#ffffff; }
.bbs_gallery li:hover p{color:#eeeeee; }
.bbs_gallery li:hover span{color:#eeeeee; }
