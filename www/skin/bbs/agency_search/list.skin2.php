...
var get_sel_no = <?=$_GET[sel]?>;		// get변수로 넘어온 맵넘버가 있다면 저장
	...
	if (get_sel_no != "") {
		...
		//$("#mview_"+get_sel_no).css("z-index","700").css("opacity","0.8");
		$("#mview_"+i).show();
		...
		<div class="map">
			<li style="background:#fff url(/images/map/map.jpg) no-repeat; width:280px; height:350px; float:left;">
				<ul style="position:relative; z-index:655;">
					...
					<a onmouseover="mview(7)" style='cursor:hand' href="/bbs/board.php?tbl=<?=$Table?>&selcity=US&sel=7" title="울산">
						<li style="position:absolute; width:20px; height:10px; margin-left:210px; margin-top:195px; "></li>
					</a><!--울산-->

					<a onmouseover="mview(8)" style='cursor:hand' href="/bbs/board.php?tbl=<?=$Table?>&selcity=GN&sel=8" title="경상남도">
						<li style="position:absolute; width:40px; height:30px; margin-left:150px; margin-top:200px;  "></li></a><!--경상남도-->
				...
				<tr>
					<td class="view">
						<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
							...
							<tr>
								<td align="center" class="viewfont" style="padding:7px 0 5px 0;">&nbsp;</td>
								<td align="center" class="viewfontao" style="padding:7px 0 5px 0;">대리점명</td>
								<td align="center" class="viewfontvo" style="padding:7px 0 5px 0;">주소</td>
								<td align="center" class="viewfonteo" style="padding:7px 0 5px 0;">전화</td>
							</tr>
						</table>
					...
					<td class="view01">
						<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
							...
							<tr valign="top">
								...
								<td   align="center" style="padding:2px 0 5px 0;"><?if($_SESSION[userlevel] >= 90){?><a href='/bbs/board.php?tbl=<?=$Table?>&mode=MODIFY&num=<?=$list[$i][b_no]?>'><? } ?><?=$list[$i][b_subject]?><?if($_SESSION[userlevel] >= 90){?></a><? } ?></td>
								<td  align="center" style="padding:2px 0 5px 0;"><?=$list[$i][b_ex1]?></td>
