...
<style>
.tit {font-size:13px; font-weight:bold;color:#464646;text-align:center;}
...
.partner_wrap{width:100%; position:relative; overflow:hidden; }
.partner_box{width:180px; height:60px; border:1px solid #ddd; float:left; margin-left:20px; margin-bottom:30px}
.partner_box:nth-child(4n+1){margin-left:0; }
</style>
...
<div class="partner_wrap">
	...
	<div class="partner_box"><a href="<?=($LogAdmin==TRUE)?"/bbs/board.php?tbl=$Table&mode=MODIFY&num={$list[$i][b_no]}":$list[$i]["b_link1"]?>" target="<?=($LogAdmin==TRUE)?"_self":"_blank"?>"><img src="<?=($list[$i]["img_1"])?>" width=190 height=60></a><?if($LogAdmin==TRUE){?><input type=checkbox name='list_ck[]' value='<?=$list[$i]["b_no"]?>' style="vertical-align:middle;border:0;"><?}?></div>
	<? } ?>
	<?if($i == 0){?>등록된 게시물이 없습니다.<?}?>
</div>
