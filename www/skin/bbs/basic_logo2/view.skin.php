...
<style type="text/css">
img { border:0; }
.tit {font-weight:bold;color:#44444;text-align:center; }
...
.bbs_top li.bg_none{background:none; margin-left:0; padding-left:0; padding-left:20px; }
...
<table width="<?=$Board_Admin["width"]?>" border="0" align="center">
	<tr>
		<td class="bottom_line2"><strong style="padding-left:15px; color:#333">댓글 <span style="font:size:9pt; color:#325f95;"><?=$comm_total?>개</span></strong></td>
	</tr>
						...
						<div style="width:*; float:left;">
							...
							if($comm[$i]["c_ex10"]=="1") {
								if ($_SESSION[userid] == $comm[$i]["c_member"]  || $_SESSION[userid] == $view["b_member"] || $_SESSION[userid] == $comm[$i]["TopCom_writer"]) {
									...
									<span style="color:navy;padding-left:10px;cursor:pointer;" onclick="menu('comment2_<?=$i?>');">댓글</span>
									<?
								} else {
									echo "비밀글 입니다.";
								}
							...
							/<span  style="color:navy;padding-left:3px;cursor:pointer;" onclick="menu('comment_<?=$i?>');">수정</span><? }?>
							<? if($comm[$i]["comdele"]==TRUE){?>
							/<span  style="color:navy;padding-left:3px;cursor:pointer;" onclick="ComdeleteChk('<?=$comm[$i]["comdele"]?>');">삭제</span>
							<? }?>&nbsp;
						</div>
					</td>
