...
// 여기에서 영문으로 echo 하여 Request 된 값을 Javascript 에서 한글로 메세지를 출력함
		...
		$row[itop_opt1] = iconv("euc-kr","utf-8",$row[itop_opt1]);
		
		if($row[itop_stock] == 0 && $row[itop_stock] != "") echo "품절";
		else echo $row[itop_opt1]."|".$row[itop_stock]."|".$row[itop_no]."|".$row[itop_amount_op].$row[itop_amount];
?>